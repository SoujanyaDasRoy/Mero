...
function AndroidKeyboard({ dark = false }) {
  let _k = 0;
  const K = dark
    ? { plate: '#1D1B20', key: '#2B2930', mod: '#36343B', accent: '#4A4458', enter: '#D0BCFF', glyph: '#E6E0E9', enterGlyph: '#381E72' }
    : { plate: MD_C.inverseOnSurface, key: MD_C.surface, mod: MD_C.surfaceVariant, accent: MD_C.secondaryContainer, enter: MD_C.primaryFixedDim, glyph: MD_C.onPrimaryContainer, enterGlyph: MD_C.onPrimaryContainer };
  const key = (l, { flex = 1, bg = K.key, r = 6, minW, fs = 21, fg = K.glyph } = {}) => (
    <div key={_k++} style={{
  ...
  return (
    <div style={{
      background: K.plate, padding: '0 8px 8px',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {key('?123', { bg: K.accent, r: 100, minW: 58, fs: 14 })}
          {key(',', { bg: K.mod })}
          {key('', { flex: 3, minW: 154 })}
          {key('.', { bg: K.mod })}
          {key('', { bg: K.enter, r: 100, minW: 58, fg: K.enterGlyph })}
        </div>
      </div>
    </div>
  );
